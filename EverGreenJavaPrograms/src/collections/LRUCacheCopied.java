...
class LRUCache<K, V>{
	...
	public void printCacheAndDoublyLinkedList() {
		...
		Set<Entry<K, LRUCache<K, V>.Node<K, V>>> currSet = cache.entrySet();
		for (Entry<K, LRUCache<K, V>.Node<K, V>> entry : currSet) {
			System.out.print(entry.getKey()+":"+entry.getValue().value+" ");
		}
		System.out.println("\nLinkedList:");
		LRUCache<K, V>.Node<K, V> currNode = leastRecentlyUsed;
		while(currNode!=null){
